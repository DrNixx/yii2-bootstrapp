...
class Menu extends Widget
{
     ...
     */
    public $linkTemplate = '<a href="{url}">{label}{arrow}</a>{icon}';

    public $detailedLinkTemplate = '<a href="{url}" class="detailed"><span class="title">{label}</span><span class="details">{details}</span>{arrow}</a>{icon}';
     ...
     * This property will be overridden by the `template` option set in individual menu items via [[items]].
     */
    public $labelTemplate = '<a href="#">{label}{arrow}</a>{icon}';

    public $detailedLabelTemplate = '<a href="#" class="detailed"><span class="title">{label}</span><span class="details">{details}</span>{arrow}</a>{icon}';

    public $iconTemplate = '<span class="icon-thumbnail"><i class="{icon}"></i></span>';

    public $arrowTemplate = '<span class="arrow {open}"></span>';
    ...
    protected function renderItems($items, $level = 1)
    {
        ...
        foreach ($items as $i => $item) {
            ...
            if ($i === 0 && $level == 1 && $this->firstItemCssClass !== null) {
                $class[] = $this->firstItemCssClass;
            }

            if ($i === $n - 1 && $level == 1 && $this->lastItemCssClass !== null) {
    ...
    protected function renderItem($item)
    {
        ...
        if (!empty($item['items'])) {
            $arrow = strtr($this->arrowTemplate, [
                '{open}' => $item['open'] ? $this->openCssClass : "",
            ]);
        } else {
            $arrow = "";
        }

        $label = (!empty($icon) || !empty($arrow)) ? "<span class=\"title\">{$item['label']}</span>" : $item['label'];

        if (isset($item['url'])) {
            if (isset($item['details'])) {
                $template = ArrayHelper::getValue($item, 'template', $this->detailedLinkTemplate);
            ...
            if (isset($item['details'])) {
                $template = ArrayHelper::getValue($item, 'template', $this->detailedLabelTemplate);
    ...
    protected function normalizeItems($items, &$active)
    {
        foreach ($items as $i => $item) {
            ...
            if (!isset($item['active'])) {
                ...
                } elseif ($this->activateParents && $hasActiveChild) {
                    $active = $items[$i]['open'] = true;
                } else {
                    $items[$i]['active'] = false;
                }
            } elseif ($item['active']) {
            ...
            if (isset($item['roles']) && (sizeof($item['roles']) > 0)) {
                $can = false;
